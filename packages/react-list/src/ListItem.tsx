...
const [createListItemContext, createListItemScope] = createContextScope(
  LIST_ITEM_NAME,
  [],
);
...
const [ListItemProvider, useListItemContext] =
  createListItemContext<ListItemContextValue>(LIST_ITEM_NAME);

type ListItemHeadingProps = ListItemScopedProps<
  Omit<ComponentPropsWithRef<typeof Primitive.p>, "id">
> & {
  asChild?: boolean;
};
...
type ListItemCollapsibleContentProps = ComponentProps<
  typeof Collapsible.Content
>;

const ListItemCollapsibleContent: ForwardRefExoticComponent<CollapsibleContentProps> =
  Collapsible.Content;

const ListItem = forwardRef<ListItemElement, ListItemProps>(
  (
    props: ListItemScopedProps<ListScopedProps<ListItemProps>>,
    forwardedRef,
  ) => {
