...
export const successInputValence =
  "shadow-success-500/50 dark:shadow-success-600/50";
export const infoInputValence = "shadow-info-500/50 dark:shadow-info-600/50";
export const warningInputValence =
  "shadow-warning-500/50 dark:shadow-warning-600/50";
...
const sharedSubduedInputStyles: ComponentFragment<InputStyleProps> = (
  props,
) => [
...
const sharedDefaultInputStyles: ComponentFragment<InputStyleProps> = (
  props,
) => [
...
export const inputInput: ComponentFunction<InputStyleProps> = (
  props,
  ...etc
) =>
...
export const inputCheckbox: ComponentFunction<InputStyleProps> = (
  { size = 5 },
  ...etc
) =>
  mx(
...
export const inputCheckboxIndicator: ComponentFunction<InputStyleProps> = (
  { size = 5 },
  ...etc
) => mx(getSize(computeSize(sizeValue(size) * 0.7, 4)), ...etc);

export const inputWithSegmentsInput: ComponentFunction<InputStyleProps> = (
  props,
  ...etc
) =>
  mx(
...
export const inputLabel: ComponentFunction<InputMetaStyleProps> = (
  props,
  ...etc
) =>
  mx(
...
export const inputDescription: ComponentFunction<InputMetaStyleProps> = (
  props,
  ...etc
) => mx(descriptionText, props.srOnly && "sr-only", ...etc);

export const inputDescriptionAndValidation: ComponentFunction<
  InputMetaStyleProps
> = (props, ...etc) =>
  mx("leading-none mlb-1", props.srOnly && "sr-only", ...etc);

export const inputValidation: ComponentFunction<InputMetaStyleProps> = (
  props,
  ...etc
) =>
  mx(
    descriptionText,
    props.srOnly ? "sr-only" : valenceColorText(props.validationValence),
    ...etc,
  );
